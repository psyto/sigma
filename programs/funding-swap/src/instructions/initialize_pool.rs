...
pub fn handler(ctx: Context<InitializePool>, params: PoolParams) -> Result<()> {
    ...
    // Validate parameters
    require!(params.market_symbol.len() > 0 && params.market_symbol.len() <= 16, FundingSwapError::InvalidMarketSymbol);
    require!(params.funding_period_seconds >= 3600, FundingSwapError::DurationTooShort); // Min 1 hour
